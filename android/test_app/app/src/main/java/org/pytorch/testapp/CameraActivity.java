...
public class CameraActivity extends AppCompatActivity {
  ...
  private static final int TENSOR_WIDTH=480;
  private static final int TENSOR_HEIGHT=480;
    ...
    imageAnalysis.setAnalyzer(
        new ImageAnalysis.Analyzer() {
          ...
          public void analyze(ImageProxy image, int rotationDegrees) {
            ...
            if (BuildConfig.FORWARD_COUNT > 0 && forwardCount >= BuildConfig.FORWARD_COUNT) {
              Log.i(TAG, "AAA TPr :" + mPerfStatTensorPrep.getStatString());
              Log.i(TAG, "AAA Fwd stat:" + mPerfStatFwd.getStatString());
              return;
            }
  ...
  private PerfStat mPerfStatTensorPrep = new PerfStat();
  private PerfStat mPerfStatFwd = new PerfStat();
  ...
  protected Result analyzeImage(ImageProxy image, int rotationDegrees) {
    ...
    if (mModule == null) {
      ...
      mInputTensor = Tensor.fromBlob(mInputTensorBuffer, new long[]{1, 3, TENSOR_WIDTH, TENSOR_HEIGHT});
    }
    ...
    } else if (BuildConfig.TU_TYPE == 1) {
      PyTorchVision.imageYUV420CenterCropToFloatBuffer1(
          image.getImage(), rotationDegrees,
          TENSOR_WIDTH, TENSOR_HEIGHT,
          TensorImageUtils.TORCHVISION_NORM_MEAN_RGB,
          TensorImageUtils.TORCHVISION_NORM_STD_RGB,
          mInputTensorBuffer, 0);
    } else if (BuildConfig.TU_TYPE == 2) {
      PyTorchVision.imageYUV420CenterCropToFloatBuffer2(
          image.getImage(), rotationDegrees,
          TENSOR_WIDTH, TENSOR_HEIGHT,
          TensorImageUtils.TORCHVISION_NORM_MEAN_RGB,
          TensorImageUtils.TORCHVISION_NORM_STD_RGB,
    ...
    mPerfStatTensorPrep.add(tensorPrepDuration);
    Log.i(TAG, String.format("III tensorPrepDrtn:%d", tensorPrepDuration));

    final long moduleForwardStartTime = SystemClock.elapsedRealtime();
    final Tensor outputTensor = mModule.forward(IValue.from(mInputTensor)).toTensor();
    final long moduleForwardDuration = SystemClock.elapsedRealtime() - moduleForwardStartTime;
    mPerfStatFwd.add(moduleForwardDuration);
    Log.i(TAG, String.format("III fwdDrtn:%d", moduleForwardDuration));
  ...
  protected void handleResult(Result result) {
    ...
    String message = String.format("III %d fwdDrtn:%d class:%s",
        forwardCount,
